...
class robot:
    ...
    def rand_new(self):
        ...
        num1 = random.random()
        temp = np.floor(random.random()*10)
        num2 = (-1)**temp
        return num1*num2
    ...
    def sense(self):
        ...
        for x,y in self.landmarks:
            dist = round(np.sqrt((self.x-x)**2 + (self.y-y)**2),1)
            if dist <= self.measurement_range:
                dx = round((x-self.x)+ self.rand_new()*self.measurement_noise, 2)
                dy = round((y-self.y)+ self.rand_new()*self.measurement_noise, 2)
